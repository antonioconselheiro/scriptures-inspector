...
export class ProjectService {
  ...
  createIfNotExistsWordMetadata(
    ...
    segments.forEach(segment => {
      const key = this.castSegmentIntoMetadataIndex(data, segment);
      if (!metadata[key]) {
        metadata[key] = {
          kind: '',
  ...
  getScriptureMetadataDefinedKind(
    ...
    const metadataKey = this.castSegmentIntoMetadataIndex(data, segment)
  ...
  updateScripturesMetadata(
    ...
    const metadataIndex = this.castSegmentIntoMetadataIndex(data, segment);
  ...
  getScriptureMetadataWordOfGod(
    ...
    const segment = this.castSegmentIntoMetadataIndex(project, segments[0]);
