...
export async function loadSourceBookFn<T = SourceBook>(httpClient: HttpClient, project: Project, source: string, book: string): Promise<T | null> {
  ...
  if (/^http/.test(resourcePath)) {
    return firstValueFrom(httpClient.get<T>(resourcePath)).catch(e => {
      console.error(e);
      return null;
    });
  } else {
    return readJsonFileFn<T>(resourcePath);
  }
}
