...
for (let index = 0; index < books.length; index++) {
  ...
  for (let index = 0; index < bibleMetadata[book].length; index++) {
    details[book][index] = [];

    loadChapterQueuee.push(async () => {
      if (['Jonah', 'Hebrews', '3 John', 'Revelation'].includes(book) && index === 0) {
      ...
      try {
        const json = await fetch(`/data/${encodedBook}/${encodedBook}_chapter_${bibleMetadata[book][index]}.json`).then(res => res.json())
        details[book][index].push(json);
...
async function crawle(queuee) {
  for (const func of queuee) {
    let sucess = false;
    while (!sucess) {
      try {
        ...
        console.log('Erro, tentando novamente:', error);
        sleep(3000)
      }
    }
  }
...
crawle(loadChapterQueuee).then(() => console.info(JSON.stringify(details)));
